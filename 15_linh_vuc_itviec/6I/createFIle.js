...
const crawData = async (payload) => {
  try {
    ...
    await page.type('#user_email', 'waxovi4437@regishub.com');
...
const getListFileName = async () => {
  try {
    ...
    return files.filter(name => name !== '6I.json' && name !== 'createFIle.js' ); 
  } catch (err) {
    throw err; 
  }
}
